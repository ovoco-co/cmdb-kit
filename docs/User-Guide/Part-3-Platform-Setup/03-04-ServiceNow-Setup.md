...
- **Auto-numbered types.** Change Request and Incident use ServiceNow's auto-numbered fields (CHG0001234, INC0001234). The adapter uses the `number` field for matching instead of `name`. Keep this in mind when referencing these types from other data files.
...
**Auto-generated numbers.** Change Request and Incident records in ServiceNow use auto-numbered fields (CHG0001234, INC0001234). The adapter handles this by using the `number` field for matching. Do not include `number` values in your data files; ServiceNow generates them automatically.
