...
The CMDB tracks people in their role as CI stakeholders, not as employees. An HR system tracks salary, benefits, performance reviews, and employment status. The CMDB tracks: who owns this application, who leads this team, who is the site contact for this deployment, who authored this document, who reported this incident, and who has the clearance required to access this facility.

The boundary is clear: if the data supports a configuration management, incident management, or operational question, it belongs in the CMDB. If it supports a human resources question, it stays in the HR system.
...
The `parentOrganization` attribute is self-referential, enabling hierarchical structures. Ovoco Inc is the Company. Engineering Department and Platform Department are Departments with `parentOrganization` set to "Ovoco Inc." This hierarchy can nest as deep as needed: Company, Division, Department, Branch.
...
```json
{
  ...
  "firstName": "Alex",
  "lastName": "Chen",
  "email": "alex.chen@ovoco.example.com",
...
- `Change Request.requestedBy`: who proposed the change
- `Incident.reportedBy`: who reported the incident

This makes Person the bridge between people and operational data. When you query "who reported this incident?", you get a Person record that links to a Team, which links to an Organization. When you query "what has this person been involved in?", you can find their assessments, documents, deployments, distributions, change requests, and incident reports.
...
Instead of saying "Alex Chen owns the CRM Core application," assign ownership to the post: "Platform Engineering Lead owns the CRM Core application." The Application's owner reference points to the Post, not the Person.

When Alex Chen transfers to a new role and Jordan Lee becomes the Platform Engineering Lead, you update the Post's incumbent from Alex Chen to Jordan Lee. Every CI that references the post automatically has a new owner without changing any CI records.
...
This pattern is especially important for deployment sites. The site's point of contact is a role, not a person. The Site Personnel Assignment pattern (described in the Designing Site Deployments chapter) can reference Posts rather than Persons for the same benefit.
...
Organizations: Ovoco Inc (Company), Engineering Department (Department), Platform Department (Department).

Teams: Platform Engineering, SRE, Frontend, Data Engineering, Mobile. Each team has a lead and belongs to a department.

People: ten individuals across the five teams, each with a role and team assignment.
