...
Lookup types are the reference data that gives meaning to your CI records. When a Product record has a status of "Active" or a Server record sits in a "Production" environment, those values do not live as free-text strings. They are separate object types in the CMDB, each with a Name and a description, shared by every CI that references them. This section explains why CMDB-Kit models reference data this way, how the lookup mechanism works at the schema and import level, how to design your own lookup types, and what the extended schema ships with out of the box.
...
In a spreadsheet, you might type "Active" into a status column and hope everyone spells it the same way. In CMDB-Kit, "Active" is a record in the Product Status type. Every Product that needs to say "I am active" points to that single record. This is the same principle behind a foreign key in a relational database or a shared select list in a service management tool.

The payoff is consistency. If you decide to rename "Active" to "Operational," you change one record. Every Product referencing it reflects the change immediately. You do not need to find and update fifty scattered strings across fifty data files.
...
The alternative, giving each CI type its own status list, leads to drift. Server Status might have "Live" while Product Status has "Active" and Deployment Status has "Running," all meaning the same thing. Shared lookup types eliminate that fragmentation.
...
The file lives in the `data/` directory alongside all other data files. Its name follows the standard kebab-case convention: the type "Product Status" becomes `product-status.json`, "Environment Type" becomes `environment-type.json`.
...
This is why lookup types must be imported before the CI types that reference them. The LOAD_PRIORITY array in `tools/lib/constants.js` enforces this by listing all lookup types (22 in extended, 44 in enterprise) at the top:
...
Status lookups describe where something is in its lifecycle: Product Status, Version Status, Deployment Status, Baseline Status, License Status.
...
Other qualifiers appear when status or type does not fit: Baseline Status, License Type, Deployment Role.

When creating a new lookup type, pick a name that makes the relationship clear when read as a sentence: "This Product has a Product Status of Active." "This Server has an Environment Type of Production." If the name reads naturally as "[CI] has a [Lookup Type] of [Value]," you have a good name.

## When to Create a New Lookup Type vs Adding Values

Create a new lookup type when you need a distinct set of values that would not make sense mixed into an existing type. If your organization tracks risk levels on assessments, and the values (Low, Medium, High, Critical) overlap with Priority but carry different descriptions and different semantics, create a separate "Risk Level" type rather than reusing Priority.
...
Tracks where a product is in its lifecycle, from initial planning through retirement.

| Name | Description |
|------|-------------|
| Active | Product is live and serving traffic |
| Planned | Product is approved but not yet built |
| Deprecated | Product is being phased out |
| Retired | Product has been decommissioned |

A product begins as "Planned" when it is approved. It moves to "Active" at launch. When a replacement is identified, it becomes "Deprecated." After decommissioning, it is "Retired." In the OvocoCRM example, the main product has an Active status, while a legacy reporting module might be Deprecated.
...
Tracks execution of deployments.
...
The Deployment type references this lookup. A deployment that has been approved but not yet executed is "Planned." Once execution begins, it is "In Progress." The terminal states are "Completed," "Rolled Back," or "Failed."
...
| Name | Description |
|------|-------------|
| Site Commander | Overall site authority and decision-maker |
| Operations Lead | Day-to-day operational management |
| System Administrator | System configuration and maintenance |
| Database Administrator | Database operations and tuning |
| Network Engineer | Network infrastructure and connectivity |
| Security Officer | Security compliance and access control |
| Training Coordinator | User training and onboarding |
| Help Desk Lead | End-user support coordination |
...
Add values when your process requires a distinction the defaults do not capture. If your deployment process needs a "Paused" state between "In Progress" and "Completed," add "Paused" to Deployment Status with a clear description.
