...
CMDB-Kit organizes its files around a simple principle: every file name is derivable from the type name it represents. Schema files define types, attribute files define fields, and data files contain records. This section explains the repository layout, the naming conventions, the JSON data formats, and how to keep everything consistent as the schema grows.
