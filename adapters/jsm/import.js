...
// CLI
// ---------------------------------------------------------------------------
const VALID_MODES = ['sync', 'create', 'update', 'schema', 'data'];
...
async function resolveDefaultIcon() {
  ...
  // Cloud: use ICON_ID env var if set (avoids broken default icons in type tree)
  if (config.isCloud && config.iconId) {
    defaultIconId = config.iconId;
    console.log(`  Icon: using ICON_ID override (${defaultIconId})`);
...
async function cacheRefTypes() {
  try {
    const refTypes = await api.get('/config/referencetype');
    const list = Array.isArray(refTypes) ? refTypes : (refTypes.values || []);
    for (const rt of list) {
      refTypeIds[rt.name.toLowerCase()] = String(rt.id);
    }
  } catch (_e) { /* fall through */ }
}

async function getOrCreateRefType(name) {
  const key = name.toLowerCase();
  if (refTypeIds[key]) return refTypeIds[key];
  // Create custom reference type
  try {
    const res = await api.post('/config/referencetype', { name, description: name });
    refTypeIds[key] = String(res.id);
    console.log(`  + Created reference type: ${name} (ID: ${res.id})`);
    return refTypeIds[key];
  } catch (_e) {
    console.log(`  ! Failed to create reference type: ${name}`);
    // Fall back to first available
    const fallback = Object.values(refTypeIds)[0];
    return fallback || '1';
  }
}

function resolveRefTypeName(attrKey) {
  // Check ref-type-map.json for this attribute name, fall back to _default
  if (refTypeMap[attrKey]) return refTypeMap[attrKey];
  return refTypeMap._default || 'Reference';
}

async function resolveDefaultRefType() {
  if (defaultRefTypeId) return defaultRefTypeId;
  const fallbackName = refTypeMap._default || 'Reference';
  defaultRefTypeId = await getOrCreateRefType(fallbackName);
...
async function createObjectType(schemaId, name, parentName = null, iconId = null, description = '') {
  const resolvedIcon = resolveIconForType(name, iconId);

  if (objectTypeIds[name]) {
    // If we have a mapped or override icon, force-update existing types
    if (iconMap[name] || (config.isCloud && config.iconId)) {
      const updatePayload = { name, iconId: resolvedIcon || await resolveDefaultIcon() };
      if (description) updatePayload.description = description;
      try {
        await api.put(`/objecttype/${objectTypeIds[name]}`, updatePayload);
        console.log(`  ~ Updated: ${name} (icon: ${updatePayload.iconId})`);
      } catch (_e) {
        console.log(`  ! Failed to update icon for ${name}`);
      }
    } else {
      if (description) {
  ...
  payload.iconId = resolvedIcon || await resolveDefaultIcon();
...
async function syncStructure(schemaId) {
  ...
  if (config.isCloud && !config.iconId && Object.keys(iconMap).length === 0) {
    console.log('  Note: Cloud icons may not render. Set ICON_ID or create icon-map.json in your schema directory.');
  }
      ...
      try {
        ...
        if (typeIdx === 1 && attrDef.referenceType) {
          ...
          if (refTypeId) {
            ...
            payload.additionalValue = await getOrCreateRefType(resolveRefTypeName(attrKey));
          } else {
...
async function main() {
  ...
  // Load optional icon map for per-type icon assignment
  const iconMapPath = path.join(config.schemaDir, 'icon-map.json');
  iconMap = loadJsonFile(iconMapPath) || {};
  if (Object.keys(iconMap).length > 0) {
    console.log(`  Icon map: ${Object.keys(iconMap).length} type mappings loaded`);
  }

  // Load optional reference type map for per-attribute relationship types
  const refTypeMapPath = path.join(config.schemaDir, 'ref-type-map.json');
  refTypeMap = loadJsonFile(refTypeMapPath) || {};
  await cacheRefTypes();
  if (Object.keys(refTypeMap).length > 0) {
    const count = Object.keys(refTypeMap).filter(k => k !== '_default').length;
    console.log(`  Ref type map: ${count} attribute mappings loaded (default: ${refTypeMap._default || 'Reference'})`);
  }
  ...
  if (!dryRun && mode !== 'data') await syncAttributes(schemaId);
